...
let latestFirst = localStorage.getItem("latestFirst") != null ? JSON.parse(localStorage.getItem("latestFirst")) : false;
if (latestFirst) {
    let sortButton = getElementById("sort");
    if (sortButton != null)
...
document.addEventListener("submit", (event) => {
    ...
    let newTask = getElementById("newtask");
    if (newTask != null) {
        if (newTask.value === "")
            return;
...
// --------------------------- showCompletedTasks -------------------------------
function showCompletedTasks(id) {
...
function buttonSelected(id) {
    ...
    if (buttons != null) {
        let buttonList = Object.values(buttons.children);
